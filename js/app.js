...
const showProductDetails = (product_id) => {
   ...
   fetch(`https://fakestoreapi.com/products/${product_id}`)
      ...
      .then((data) => {
         console.log(data);
         showProductDetailsInModal(data)});
};
...
const updateTotal = () => {
   const grandTotal =
      ...
      console.log(parseFloat(grandTotal.toFixed(2)));
...
document.getElementById("search-btn").addEventListener("click", function () {
   ...
   const inputFieldLower = inputField.toLowerCase();
   const searchedProduct = arr[0].filter( p => 
      p.title.toLowerCase().includes(inputFieldLower)
   );
